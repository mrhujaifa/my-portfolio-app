...
const navItems = [
  { name: "Home", icon: <FaHome />, path: "/" },        // route path
...
export default function Navbar() {
  ...
  // এখানে logic:
  // Home active যদি (route path == '/') অথবা (#home section active) হয়
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 px-4 py-2">
      <div className="container mx-auto flex items-center justify-between bg-black/50 backdrop-blur-md rounded-full px-6 py-2 shadow-lg">
        {/* Logo */}
        <div className="text-white font-bold text-xl flex items-center gap-2">
          <img src="/logo.png" alt="Logo" className="w-8 h-8 object-contain" />
          <span>Hujaifa.dev</span>
        </div>
        ...
        <div className="hidden md:flex items-center gap-4 text-white">
          <a
            href="/resume.pdf"
            download
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-400 to-yellow-300 text-black rounded-full font-medium hover:scale-105 transition"
          >
            <FaDownload />
            Resume
          </a>
      ...
      {/* Mobile Dropdown */}
      {menuOpen && (
        <div className="md:hidden mt-2 bg-black/60 backdrop-blur-md rounded-xl py-3 px-4 text-white shadow-lg space-y-2 animate-fade-in-down">
          {navItems.map((item, idx) => (
            <a
              key={idx}
              href={item.path}
